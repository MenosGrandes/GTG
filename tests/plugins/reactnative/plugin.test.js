...
import { ReactNativePlugin } from "../../../config/plugins/reactnative/index.js";
...
describe("ReactNativePlugin.extractNames", () => {
  ...
  it("ignores library imports", () => {
    const code = 'import { render, screen } from "@testing-library/react-native";';
    expect(plugin.extractNames(code)).toEqual([]);
  });

  it("ignores lowercase tags", () => {
...
`;
    ...
    expect(names).toContain("Clicker");
...
<Clicker initial={0} />
`;
    expect(plugin.extractNames(code).filter((n) => n === "Clicker").length).toBe(1);
  });
});
