...
describe("JSX Full Pipeline (justfile)", () => {
  beforeAll(() => {
  ...
  it("scaffold directory is assembled", () => {
    expect(existsSync(resolve(SCAFFOLD_DIR, "package.json"))).toBe(true);
    expect(existsSync(resolve(SCAFFOLD_DIR, "jest.config.cjs"))).toBe(true);
    expect(existsSync(resolve(SCAFFOLD_DIR, "index.html"))).toBe(true);
    expect(existsSync(resolve(SCAFFOLD_DIR, "vite.config.js"))).toBe(true);
