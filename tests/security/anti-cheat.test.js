...
describe('Anti-cheat mechanisms', () => {
  beforeAll(() => {
    ...
    if (!existsSync(pdfPath)) {
      try {
        execSync(
          'bash -c "source /app/lmod/lmod/init/bash && module load texlive/2024 && make compile_pdf SEED=1 COUNT=3 UV=true"',
          { cwd: root, stdio: 'pipe', timeout: 120000 }
        );
      } catch {
        // PDF build not available in this environment (no texlive/lmod)
      }
    }
  });
  ...
  it('integrity check catches tampering', () => {
    ...
    const tampered = content.slice(0, tamperOffset) +
